...
if config.area == "MEXICO":
    ...
    config.height, config.width = 128, 128
else:
    print("Not Implement")
...
logging.basicConfig(level=logging.INFO,
                    filename=os.path.join(base_dir, '{}_dataset_{}.log'.format(timestamp, config.index)), filemode='a',
...
if config.method == "GraphTransformer":
    ...
    model = MTGNN(adj=adj, gcn_true=True, build_adj=False, kernel_set=[7, 7], kernel_size=7, gcn_depth=1,
                  num_nodes=adj.shape[0], dropout=0.3, subgraph_size=20, node_dim=8, dilation_exponential=2,
                  conv_channels=2, residual_channels=2, skip_channels=4, end_channels=8, seq_length=config.in_len,
                  in_dim=1, out_dim=config.out_len, layers=2, propalpha=0.5, tanhalpha=3, layer_norm_affline=False)
...
for epoch in train_process:
    ...
    for train_step, (datas, data_ob_masks, data_gt_masks, labels, label_masks) in enumerate(train_dloader):
        datas, data_ob_masks, data_gt_masks, labels, label_masks = datas.float().to(device), data_ob_masks.to(
