...
class SpatialTemporalEncoding(nn.Module):
    def __init__(self, config, low_bound, high_bound) -> None:
        ...
        self.spatial_encoding = GCN(self.config.hidden_channels, self.config.hidden_channels,
                                    self.config.hidden_channels // 4, 3)
        self.time_encoding = LinearAttentionTransformer(dim=self.config.hidden_channels, depth=1, heads=1,
                                                        max_seq_len=16, n_local_attn_heads=0, local_attn_window_size=0)
        self.diffusion_embedding = DiffusionEmbedding(num_steps=config.num_steps,
        ...
        # 加载辅助数据（根据实际数据路径调整）
        self.is_sea = torch.from_numpy(np.load(f'./data/{config.area}/is_sea.npy')).to(self.device)
        self.mean = torch.from_numpy(np.load(f'./data/{config.area}/mean.npy')).to(self.device)
        self.std = torch.from_numpy(np.load(f'./data/{config.area}/std.npy')).to(self.device)
    ...
    def forward(self, x, mask, adj, diffusion_step):
        ...
        # 边界裁剪
        y = torch.clamp(y, self.low_bound, self.high_bound)
        return y
