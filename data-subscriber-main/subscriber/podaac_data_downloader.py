...
# 获取当前脚本所在的目录（即subscriber文件夹的路径）
subscriber_dir = os.path.dirname(os.path.abspath(__file__))
# 获取subscriber文件夹的父目录（即data-subscriber-main目录）
parent_dir = os.path.dirname(subscriber_dir)
# 将父目录添加到Python的搜索路径（让Python能找到subscriber模块）
